...
export default function ScheduleListPage() {
  ...
  const [cfgConfigured, setCfgConfigured] = useState(false);
  const [cfgSaving, setCfgSaving]       = useState(false);
  const [cfgShowSess, setCfgShowSess]   = useState(false);
  ...
  useEffect(() => {
    ...
    fetch("/api/scheduler-config")
      .then((r) => r.json())
      .then((d) => setCfgConfigured(d.configured ?? false))
      .catch(() => {});
  }, []);
  ...
  async function saveConfig(e) {
    ...
    try {
      const res = await fetch("/api/scheduler-config", {
        ...
        body: JSON.stringify({ session: cfgSession, authToken: cfgToken }),
      });
      if (res.ok) {
        setCfgConfigured(!!(cfgSession && cfgToken));
        setCfgSession("");
        setCfgToken("");
        setShowConfig(false);
      }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      {/* Modal de configuração do scheduler */}
      {showConfig && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60" onClick={() => setShowConfig(false)} />
          <div className="relative bg-gray-900 border border-gray-700 rounded-xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-sm font-semibold text-white mb-1">Configuração do Agendador</h2>
            <p className="text-xs text-gray-500 mb-5">
              Credenciais usadas pelo cron para abrir os chamados. Obtidas em <span className="font-mono text-gray-400">Application → Cookies</span> no DevTools.
            </p>
            <form onSubmit={saveConfig} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <input
                    type={cfgShowSess ? "text" : "password"}
                    value={cfgSession}
                    onChange={(e) => setCfgSession(e.target.value)}
                    placeholder="Cole o valor do cookie SESSION"
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 pr-10 text-xs text-gray-100 font-mono focus:outline-none focus:border-blue-500"
                  />
                  <button type="button" onClick={() => setCfgShowSess((v) => !v)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 text-xs">
                    {cfgShowSess ? "ocultar" : "ver"}
                  </button>
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-400 mb-1 block">HYPER-AUTH-TOKEN</label>
                <div className="relative">
                  <input
                    type={cfgShowTok ? "text" : "password"}
                    value={cfgToken}
                    onChange={(e) => setCfgToken(e.target.value)}
                    placeholder="Cole o valor do HYPER-AUTH-TOKEN"
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 pr-10 text-xs text-gray-100 font-mono focus:outline-none focus:border-blue-500"
                  />
                  <button type="button" onClick={() => setCfgShowTok((v) => !v)}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 text-xs">
                    {cfgShowTok ? "ocultar" : "ver"}
                  </button>
                </div>
              </div>
              <div className="flex gap-3 pt-1">
                <button type="submit" disabled={cfgSaving || !cfgSession || !cfgToken}
      ...
      <div className="border-b border-gray-800 px-4 sm:px-6 py-3 flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          title="Configurar autenticação do agendador"
          className={`p-1.5 rounded-lg border transition-colors ${
            cfgConfigured
              ? "border-green-800/50 text-green-500 hover:bg-green-900/20"
