...
export async function lookupDelegationUserId(client, ticketId, userName) {
  try {
    ...
    // Exact match first, then starts-with (handles trailing " -" etc.)
    let idx = suggestions.findIndex((s) => s.toLowerCase().trim() === nameLower);
    if (idx < 0) idx = suggestions.findIndex((s) => s.toLowerCase().trim().startsWith(nameLower));
