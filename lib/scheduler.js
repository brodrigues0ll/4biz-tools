  ...
  cron.schedule("* * * * *", async () => {
    try {
      ...
      for (const schedule of due) {
        ...
        executeSchedule({ ...schedule.toObject(), session: cfg.session, authToken: cfg.authToken })
          .catch((err) => console.error(`[scheduler] Erro ao executar ${schedule._id}:`, err.message));
      }
    } catch (err) {
