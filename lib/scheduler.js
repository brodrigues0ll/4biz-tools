...
async function tryDelegate(client, ticketId, tecnico, groupId) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      const userId = tecnico.usarId && tecnico.id
        ? tecnico.id
        : await lookupDelegationUserId(client, ticketId, tecnico.nome);
      if (!userId) {
...
export async function initScheduler() {
  ...
  // ── Log de resumo ao iniciar ───────────────────────────────────────────────
  try {
    await connectDB();
    const ativos = await Schedule.find({ ativo: true }).lean();
    if (ativos.length === 0) {
      console.log("[scheduler] Nenhum agendamento ativo.");
    } else {
      const grupos = {};
      for (const s of ativos) {
        const dt   = new Date(s.proximaExecucao);
        const chave = dt.toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: process.env.TZ || "America/Sao_Paulo" });
        const qtd  = s.todasUnidades ? 1 : (s.unidades?.length || 1);
        if (!grupos[chave]) grupos[chave] = { dt, total: 0 };
        grupos[chave].total += qtd;
      }
      const ordenado = Object.entries(grupos).sort((a, b) => a[1].dt - b[1].dt);
      for (const [chave, { total }] of ordenado) {
        console.log(`[scheduler] ${total} chamado(s) agendado(s) para ${chave}`);
      }
    }
  } catch (e) {
    console.warn("[scheduler] Erro ao gerar resumo inicial:", e.message);
  }
